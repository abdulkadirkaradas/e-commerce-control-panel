    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-details.update", [$productDetail->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product_uuid') ? 'is-invalid' : '' }}" name="product_uuid_id" id="product_uuid_id">
                    @foreach($product_uuids as $id => $entry)
                        <option value="{{ $id }}" {{ (old('product_uuid_id') ? old('product_uuid_id') : $productDetail->product_uuid->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('details') ? 'is-invalid' : '' }}" type="text" name="details" id="details" value="{{ old('details', $productDetail->details) }}">
