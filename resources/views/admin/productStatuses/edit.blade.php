    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-statuses.update", [$productStatus->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="status">{{ trans('cruds.productStatus.fields.status') }}</label>
                <input class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" type="text" name="status" id="status" value="{{ old('status', $productStatus->status) }}">
